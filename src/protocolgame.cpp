...
bool ProtocolGame::canSee(int32_t x, int32_t y, int32_t z) const
{
	...
	if ((x >= myPos.getX() - 8 + offsetz) && (x <= myPos.getX() + 10 + offsetz) &&
	...
	if (creature == player) {
		if (oldStackPos >= 10) {
			...
			if (oldPos.x < newPos.x) { // east, [with new y]
				msg.addByte(0x66);
				GetMapDescription(newPos.x + 10, newPos.y - 6, newPos.z, 1, 14, msg);
...
void ProtocolGame::MoveDownCreature(NetworkMessage& msg, const Creature* creature, const Position& newPos, const Position& oldPos)
{
	...
	//east
	msg.addByte(0x66);
	GetMapDescription(oldPos.x + 10, oldPos.y - 7, newPos.z, 1, 14, msg);
